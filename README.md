...
- **国家专页**:`/countries`（及 `/en/countries`）为高商业意图落地页，现覆盖 **22 个国家/地区**：
  美国、加拿大、澳大利亚、新加坡、英国、香港、德国、日本、阿联酋、新西兰、马来西亚、法国、
  瑞士、韩国、泰国、越南、荷兰、意大利、西班牙、巴西、印度、爱尔兰。每页含覆盖服务、当地实务
  要点（执行路径/时效/税务）、处理路径与 CTA，并带 LegalService + FAQPage 结构化数据（可出富
  摘要）；数据在 `COUNTRIES` 字典中维护，新增国家加一个条目即可。首页全球网络区块已联动链接
  （中英文 href 自动切换）。
- **AI 咨询助手（chat intake）**:全站右下角对话式接待小部件（`/static/chat.js`，自包含样式与
  流程，中英双语跟随页面语言）。7 步引导采集（业务线/双方/金额/时间/材料/诉求/联系方式），
